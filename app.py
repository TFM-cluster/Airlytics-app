...
cluster_info = {
    1: {
        "text": "クラスター1：都内在住の働く中高年男女。通勤や夜のリラックスタイムにラジオを聴く。情報番組、ニュース、トーク番組を好む傾向。\n"
                "性別：男性56%、女性43%で半々。\n"
                "年代：50代～60代で43%→次に40代。\n"
    ...
    3: {
        "text": "クラスター3：20代男性会社員、昼夜問わずアクティブ聴取。情報収集やエンタメに積極的な若手社会人・学生。\n"
...
if not match.empty:
    ...
    if not others.empty:
        ...
        for _, row in pivot_data.iterrows():
            pivot_table.at[row["曜日"], row["開始時"]] = f"<a href='#cluster{cluster}'>クラスタ{cluster}</a>"
        ...
        with st.container():
            ...
            for day in weekday_order:
                ...
                for h in range(5, 30):
                    if day in pivot_table.index and h in pivot_table.columns:
                        val = pivot_table.at[day, h]
                    else:
                        val = "<span style='color:#cccccc;'>－</span>"
